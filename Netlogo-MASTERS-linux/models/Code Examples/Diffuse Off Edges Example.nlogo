...
to setup
  clear-all
  ;; we assume that vertical and horizontal wrapping are both
  ;; off, so edge and corner patches have fewer than 8 neighbors
  set edge-patches patches with [count neighbors != 8]
  set main-patches patches with [count neighbors = 8]
  ;; create some random numbers to diffuse
  ask main-patches [ set value random-float 100 ]
  recolor
  reset-ticks
end
...
to go
  ;; first, do a normal diffuse
  diffuse value 1
  ;; now remove any of the value which has reached the edges
  ;; of the world
  ask edge-patches [ set value 0 ]
  recolor
  tick
end
