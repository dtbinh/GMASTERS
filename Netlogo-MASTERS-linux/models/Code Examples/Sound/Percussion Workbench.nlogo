...
globals [ drum-index memory ]

;; initializes the drum-index and the memory bank
to startup
  set drum-index 0
  set memory (list "" "" "" "")
end
...
;; generates a NetLogo command to play the current drum
;; at the current pitch with the current velocity
to-report generate-command
  report (word "sound:play-drum "
               "\"" ( item drum-index sound:drums ) "\" "
               velocity)
end
...
;; arrow keys to scroll through the drum list
to first-drum
  set drum-index 0
  play
end
...
to last-drum
  set drum-index length sound:drums - 1
  play
end

to next-drum
  if drum-index < length sound:drums - 1 [
...
The instrument section allows you to choose one of the 47 drums provided by the extension. (For a complete list, see the Sound section of the NetLogo User Manual).
